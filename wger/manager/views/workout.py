...
@login_required
def export(request):
    workout_export_res = ExportWorkoutsResource()
    queryset = Workout.objects.filter(user=request.user)
    dataset = workout_export_res.export(queryset)
    response = HttpResponse(dataset.json, content_type='application/json')
    response['Content-Disposition'] = 'attachment; filename="workouts.json"'
    return response


class ExportWorkoutsResource(resources.ModelResource):

    class Meta:
        model = Workout
        exclude = ('user', 'id',)
        export_order = ('comment', 'creation_date',)


class ExportDayResource(resources.ModelResource):
